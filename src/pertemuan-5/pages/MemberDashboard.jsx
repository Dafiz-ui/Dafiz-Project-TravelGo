...
export default function MemberDashboard() {
  ...
  const [user, setUser] = useState({ name: "Traveler", role: "member" });

  useEffect(() => {
    const auth = getMemberAuth();
    setUser({
      name: auth.name || auth.email || "Traveler",
  ...
  const stats = {
    status: "Active",
    level: user.role === "vip" ? "VIP" : "Member",
    totalBooking: 5,
    totalTransactions: "Rp 12.500.000",
  };

  const promos = user.role === "vip"
    ? [
        { id: 1, title: "VIP 20% off - Paket Bali", desc: "Diskon eksklusif untuk member VIP" },
        { id: 2, title: "Early Access Promo", desc: "Akses lebih awal untuk paket populer" },
      ]
    : [
        { id: 3, title: "Member 10% off", desc: "Diskon bagi member biasa" },
      ];
  ...
  const bookingHistory = [
    { id: "TRX-001", paket: "Bali Sunrise", date: "2026-06-15", status: "Completed" },
    { id: "TRX-002", paket: "Lombok Romantis", date: "2026-05-02", status: "Cancelled" },
  ];

  return (
    <div className="space-y-10">
      {/* Header / Navbar */}
      <section className="rounded-3xl bg-gradient-to-r from-slate-900 to-emerald-600 text-white p-8 shadow-2xl flex items-center justify-between">
        <div>
          <p className="uppercase tracking-[0.3em] text-sm text-slate-300 mb-2">Member Area TravelGo</p>
          <h1 className="text-3xl md:text-4xl font-bold">Selamat datang, {user.name}</h1>
          <p className="text-slate-200 mt-2">{user.role === "vip" ? "Terima kasih atas dukungan VIP Anda" : "Terima kasih telah menjadi member TravelGo"}</p>
        </div>
      ...
      <section className="grid gap-6 md:grid-cols-3">
        <div className="rounded-2xl bg-white p-6 shadow border border-slate-200">
          ...
          <p className="font-medium text-slate-900 mb-3">{user.role === "vip" ? "VIP" : "Member"}</p>
          <Link to="/member/register" className="text-emerald-600 hover:underline">Edit Profil</Link>
        </div>

        <div className="rounded-2xl bg-white p-6 shadow border border-slate-200 md:col-span-2">
          <h3 className="text-lg font-semibold mb-4">Promo Member</h3>
      ...
      <section className="rounded-2xl bg-white p-6 shadow border border-slate-200">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold">Daftar Paket Travel</h3>
          <Link to="/paket-wisata" className="text-emerald-600 hover:underline">Lihat Semua</Link>
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
          {samplePackages.map((pkg) => (
            <div key={pkg.id} className="rounded-2xl p-4 border border-slate-100">
              ...
              <Link to={`/paket/${pkg.id}`} className="text-emerald-600 text-sm mt-2 inline-block">Detail</Link>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="rounded-2xl bg-white p-6 shadow border border-slate-200">
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <button className="rounded-2xl bg-emerald-600 px-4 py-2 text-white">Kirim</button>
            </div>
          </form>
        </div>
        ...
        <div className="rounded-2xl bg-white p-6 shadow border border-slate-200">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              <div className="text-2xl text-emerald-600"><FaHeadset /></div>
              <div>
                <p className="font-semibold">Live Chat</p>
                <p className="text-sm text-slate-500">Klik tombol chat di pojok kanan bawah untuk mulai</p>
              </div>
            </div>

            <div className="flex items-start gap-3">
              <div className="text-2xl text-emerald-600"><FaComments /></div>
              <div>
                <p className="font-semibold">Email Support</p>
                <p className="text-sm text-slate-500">support@travelgo.com</p>
              </div>
            </div>
          </div>
        </div>
